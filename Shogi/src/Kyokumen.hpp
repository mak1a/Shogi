...
public:
    ...
    constexpr Te(const uint32 from_,
                 const uint32 to_,
                 const uint32 koma_,
                 const uint32 capture_ = Emp,
                 const uint32 promote_ = 0,
                 const uint32 kind_ = 0,
                 const int32 value_ = 0) noexcept
    : m_from(from_), m_to(to_), m_capture(capture_), m_promote(promote_), m_koma((promote_ == 0) ? koma_ : koma_ | Promote), m_kind(kind_), m_value(value_) {}

    constexpr Te(int32 i) noexcept : Te(0, 0, 0) {}
    ...
    bool operator==(const Te& te_) const noexcept {
        return te_.GetFrom() == m_from && te_.GetTo() == m_to && te_.GetKoma() == m_koma && te_.GetPromote() == m_promote;
    }
};
...
private:
    ...
    s3d::HashTable<uint64, uint32> m_hashHistory;
    s3d::Array<std::pair<std::bitset<28>, uint64>> m_outeHistory;
    ...
    int32 EvalMin(s3d::Array<Te>& moveSelf_, s3d::Array<Te>& moveEnemy_);

    int32 EvalMax(s3d::Array<Te>& moveSelf_, s3d::Array<Te>& moveEnemy_);
...
public:
    ...
    [[nodiscard]] s3d::Array<Te> GetTeValids() const noexcept {
        return m_teValids;
    }
    ...
    [[nodiscard]] bool IsSennitite() const noexcept {
        ...
        return (std::max_element(m_hashHistory.begin(), m_hashHistory.end(), [](const auto& a, const auto& b) -> bool { return (a.second < b.second); })->second >= 4);
    }
